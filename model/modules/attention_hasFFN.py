...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if add_temp_attn:
            ...
            # Temp-Attn
            self.temp_norm1 = AdaLayerNorm(dim, num_embeds_ada_norm) if self.use_ada_layer_norm else nn.LayerNorm(dim)
            ...
            self.temp_norm2 = AdaLayerNorm(dim, num_embeds_ada_norm) if self.use_ada_layer_norm else nn.LayerNorm(dim)
            self.temp_ff = FeedForward(dim, dropout=dropout, activation_fn=activation_fn, final_dropout=final_dropout)
    ...
    def forward(
        ...
        if self.temp_attn is not None:
            ...
            hidden_states = self.temp_attn(norm_hidden_states1) + hidden_states
            ...
            hidden_states = self.temp_ff(norm_hidden_states2) + hidden_states
            # rearrange back
