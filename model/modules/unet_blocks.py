...
class TemporalConvLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim=None, num_layers=4, dropout=0.0):
        ...
        for i in range(num_layers):
            if i == num_layers - 1:
                next_dim = out_dim
            convs.extend([
                nn.GroupNorm(32, prev_dim), nn.SiLU(), nn.Dropout(dropout),
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = identity + hidden_states if video_length > 1 \
...
class UNetMidBlock3DCrossAttn(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None, attention_mask=None):
        ...
        for layer_idx in range(len(self.attentions)):
            ...
            hidden_states = attn(hidden_states, encoder_hidden_states=encoder_hidden_states)[0]
...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def forward(
        ...
        for layer_idx in range(len(self.resnets)):
            resnet, attn = self.resnets[layer_idx], self.attentions[layer_idx]
            is_checkpointing = self.training and self.gradient_checkpointing
            hidden_states = checkpoint(func=resnet, inputs=(hidden_states, temb), flag=is_checkpointing)
            if hasattr(self, "temp_convs"):
                temp_conv = self.temp_convs[layer_idx]
                hidden_states = checkpoint(func=temp_conv, inputs=(hidden_states,), flag=is_checkpointing)
            hidden_states = checkpoint(
                func=attn, inputs=(hidden_states, encoder_hidden_states), flag=is_checkpointing)[0]
